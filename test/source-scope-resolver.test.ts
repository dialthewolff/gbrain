...
describe('resolveRequestedScope — explicit source_id', () => {
  ...
  test('remote scalar grants cannot be overridden by explicit source_id', () => {
    const scalar = ctxOf({ remote: true, sourceId: 'alpha' });
    expect(() => resolveRequestedScope(scalar, 'beta')).toThrow(OperationError);
    expect(resolveRequestedScope(scalar, 'alpha')).toEqual({ sourceId: 'alpha' });
    ...
    expect(() => resolveRequestedScope(emptyGrant, 'beta')).toThrow(OperationError);
  });
});
